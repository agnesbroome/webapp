  ...
  $.ajax({
    ...
    success: function(data){
      ...
      if(data[random]['tags'].length != 0){
        $("#resultSB").append('<p>' + data[random]['tags'][0]['name'] + ' from ' + countryEng + '</p>');
      }
      else{
        $("#resultSB").append('<p>From ' + countryEng + '</p>');
      }
...
var countryX = [{'name': 'United States of America'}, {'name': 'Australia'}];
var randomNum = countryX[randomInt(0, countryX.length-1)]['name'];
var chosenC;
for (x in countries){
